...
(>>>) ::
  ...
  -> Parser b
  -> Parser b
(>>>) (P a) pb = P (\i -> case a i of
                              ErrorResult e -> ErrorResult e
                              Result l _ -> parse pb l)
...
list1 ::
  Parser a
  -> Parser (List a)
-- list1 p@(P a) = P (\i -> case i of
--                       Nil -> ErrorResult UnexpectedEof
--                       _ -> case a i of
--                             ErrorResult e -> ErrorResult e
--                             Result r v -> case parse (list p) r of
--                                             ErrorResult e' -> ErrorResult e'
--                                             Result r' v' -> Result r' (v :. v'))
list1 k = flbindParser k (\v' -> 
            flbindParser (list k) (\vv -> valueParser (v' :. vv))) 
...
sequenceParser (h :. t) = flbindParser h (\v -> 
                            flbindParser (sequenceParser t) (\vv -> valueParser (v :. vv)))
...
firstNameParser ::
  Parser Chars
firstNameParser = flbindParser upper (\c -> flbindParser (list lower) (\cs -> valueParser (c:.cs)))
...
surnameParser = flbindParser upper (\c -> 
                  flbindParser (thisMany 5 lower) (\cs -> 
                    flbindParser (list lower) (\t' -> valueParser (c :. cs ++ t'))))
