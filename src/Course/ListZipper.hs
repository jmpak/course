...
findLeft f (ListZipper l a r) =
  case (break f l) of
    (_, Nil) -> IsNotZ
    (rs', x':.ls') -> IsZ (ListZipper ls' x' (reverse rs' ++ a :. r))
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a r) = (ListZipper r' a' Nil) where (a' :. r') = reverse (a :. r)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l a Nil) = (ListZipper Nil a' l') where (a' :. l') = (reverse l) ++ (a :. Nil)
...
moveLeftN n z = if n < 0 
                then moveRightN (negate n) z
                else moveLeftN (n-1) -<< moveLeft z
...
moveRightN n z = if n < 0
                 then moveLeftN (negate n) z
                 else moveRightN (n-1) -<< moveRight z
...
moveLeftN' n z = if n < 0
                 then moveRightN' (negate n) z
                 else moveLeftN'' n z 0
                   where
                        moveLeftN'' 0 z' _ = Right z'
                        moveLeftN'' n' z' q =
                          case moveLeft z' of
                            IsNotZ -> Left q
                            IsZ z'' -> moveLeftN'' (n' - 1) z'' (q + 1)
...
moveRightN' n z = if n < 0
                 then moveLeftN' (negate n) z
                 else moveRightN'' n z 0
                   where
                        moveRightN'' 0 z' _ = Right z'
                        moveRightN'' n' z' q =
                          case moveRight z' of
                            IsNotZ -> Left q
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z@(ListZipper l _ r) = if ll >= n 
                             then moveLeftN  (ll - n) z
                             else if tl >= n
                                  then moveRightN ((ll + lr + 1) - n) z
                                  else IsNotZ
                            where 
                              ll = (length l)
                              lr = (length r)
